...
mongoose.connect('mongodb+srv://admin-safadi:safadi95@cluster0.zompqse.mongodb.net/blogDB'), { useNewUrlParser: true };

//Making a schema that includes title&content for each post
let postsSchema = {
  title: String,
  content: String
};
...
//--------------------------------------------------
//need to save all current and new posts in a document - done saved each const and put them in thier default pages -done
//find a way to parse current posts, and new ones -done
//store them -done
//find a way to view them in the main page -done
//connect the show more to each post ID and view it in a new post page *

app.get("/", function (req, res) {
  Post.find({}, function (err, foundPosts) {
    if (!err) {
      console.log("found posts will be shown in home");
      res.render("home", { startingContent: homeStartingContent.content, foundPosts: foundPosts });
    }
  })
...
app.post("/compose", function (req, res) {
  //parse the post, save it in db and show it in home & post pages

  const title = req.body.postTitle;
  const content = req.body.postBody;


  Post.create({ title: title, content: content }, function (err) {
    if (err) {
      console.log(err);
    } else {
      res.redirect("/");
      console.log("redirecting and saving to db");
    };
  });
...
app.get("/posts/:postId", function (req, res) {
  ...
  Post.findOne({ _id: requestedPostId }, function (err, foundPost) {
    if (!err) {
      console.log(`Found post with ID: ${requestedPostId}`);
